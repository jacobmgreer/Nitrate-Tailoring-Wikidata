...
files <- list.files("SPARQL/records-by-instance", pattern = "\\.sparql$", recursive = TRUE, full.names = TRUE)
...
dbExecute(con, "
  COPY (
    SELECT DISTINCT db, item, updated
    FROM records
  )
  TO 'release/records-by-instance.parquet' (FORMAT PARQUET);
")
